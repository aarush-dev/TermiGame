...
class GameWindow(TermiGame.window.Window):
	def __init__(self, *args, **kwargs):
		...
		self.title = TermiGame.label.Label(window=self, text=["   < == SPACESHIP == >   "], x=self.width // 2, y=15, anchor="center", color_pair=((45, 52, 54), (255, 234, 167)))
		self.help = TermiGame.label.Label(window=self, text=[" Arrow Keys - Controls | Shift - Boost | Space - Shoot "],
		...
		self.player = player_obj.Player(window=self, x=self.width // 2, y=self.height - 4, direction=(0, 0), speed=(120, 60),
			images=TermiGame.util.load_images("D:\PycharmProjects\TermiGame\examples\spaceship\\resources\spaceship"), color_pair=((129, 236, 236), None))

		asteroid_imgs = TermiGame.util.load_images("D:\PycharmProjects\TermiGame\examples\spaceship\\resources\\asteroid")
		self.asteroids = []
		for i in range(10):
		...
		self.explosion_imgs = TermiGame.util.load_images("D:\PycharmProjects\TermiGame\examples\spaceship\\resources\explosion")
		self.explosions = []

		self.score = TermiGame.label.Label(window=self, text=["SCORE: {}".format(self.player.score)], x=0, y=self.height - 2, color_pair=((255, 255, 255), None))
		self.bullets_left = TermiGame.label.Label(window=self, text=["BULLETS LEFT: {}".format(self.player.bullets_count)], x=0, y=self.height - 3, color_pair=((255, 255, 255), None))
			...
			if self.player.state == "dead":
				...
				for ast in self.asteroids:
					...
					if self.player.state == "dead":
						...
						TermiGame.sprite.Sprite(window=self, x=self.player.x, y=self.player.y, direction=(0, 0), speed=(0, 0), images=self.explosion_imgs, image_num=0, color_pair=((255, 255, 0), None), group=self.explosions)
...
if __name__ == "__main__":
	window = GameWindow(width=140, height=55, fps=120)
